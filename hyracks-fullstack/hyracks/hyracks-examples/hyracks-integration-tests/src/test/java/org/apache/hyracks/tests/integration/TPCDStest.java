...
import org.apache.hyracks.dataflow.std.join.OptimizedHybridHashMultipleJoinOperatorDescriptor;
import org.apache.hyracks.dataflow.std.join.SampleForMultipleJoinOperatorDescriptor;
import org.apache.hyracks.dataflow.std.result.ResulWriterOnlyBufferOperatorDescriptor;
...
public class TPCDStest extends AbstractIntegrationTest {
    ...
    //test for sample webSale before join
    @Test
    public void webSaleSampleCustomerCIDHybridHashJoin() throws Exception {
        JobSpecification spec = new JobSpecification();
        long startTime = new Date().getTime();


        FileSplit[] webSaleSplits = new FileSplit[] { new FileSplit(NC1_ID, new FileReference(new File(
                "data/tpch0.001/web_sales1g.tbl"))) };
        IFileSplitProvider webSalesSplitsProvider = new ConstantFileSplitProvider(webSaleSplits);


        FileScanOperatorDescriptor webSaleScanner = new FileScanOperatorDescriptor(spec, webSalesSplitsProvider,
                new DelimitedDataTupleParserFactory(new IValueParserFactory[] { UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        ...
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE }, '|'), webSaleDesc);
        PartitionConstraintHelper.addAbsoluteLocationConstraint(spec, webSaleScanner, NC1_ID);


        SampleForMultipleJoinOperatorDescriptor sampler= new SampleForMultipleJoinOperatorDescriptor(spec,webSaleDesc,40,new int[] {3}, new IBinaryHashFunctionFactory[]{PointableBinaryHashFunctionFactory.of(UTF8StringPointable.FACTORY)},5);


        PartitionConstraintHelper.addAbsoluteLocationConstraint(spec, sampler, NC1_ID);
        ...
        spec.connect(custJoinConn, webSaleScanner, 0, sampler, 0);

        IConnectorDescriptor joinPrinterConn = new OneToOneConnectorDescriptor(spec);
        spec.connect(joinPrinterConn, sampler, 0, printer, 0);

        spec.addRoot(printer);
        File temp=new File("samp");
        runTestAndStoreResult(spec, temp);
        long endTime = new Date().getTime();
        System.out.println("it run for websale" + (endTime - startTime)
                + " 。");


    }
    @Test
    public void storeSaleSampleCustomerCIDHybridHashJoin() throws Exception {
        JobSpecification spec = new JobSpecification();
        long startTime = new Date().getTime();
        ...
        FileScanOperatorDescriptor storeSaleScanner = new FileScanOperatorDescriptor(spec, webSalesSplitsProvider,
                new DelimitedDataTupleParserFactory(new IValueParserFactory[] { UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE,
                        UTF8StringParserFactory.INSTANCE, UTF8StringParserFactory.INSTANCE}, '|'), webSaleDesc);
        ...
        IConnectorDescriptor custJoinConn = new OneToOneConnectorDescriptor(spec);
        spec.connect(custJoinConn, storeSaleScanner, 0, sampler, 0);
        ...
        runTestAndStoreResult(spec, temp);
        long endTime = new Date().getTime();
        System.out.println("it run for customer" + (endTime - startTime)
                + " 。");
    ...
    @Test
    public void testBFsplitFirstGetBF() throws Exception {
        ...
        IOperatorDescriptor printer[]= new IOperatorDescriptor[2];
        for (int i = 0; i < outputArity; i++) {
            ...
            if (i==0) {
                ...
                printer[i]=new ResulWriterOnlyBufferOperatorDescriptor(spec, rsId, false, false,
                        ResultSerializerFactoryProvider.INSTANCE.getResultSerializerFactoryProvider());
            }
    ...
    @Test
    public void testBFsplitSecondMergeBF() throws Exception {
        //This is the join without order and wait for the sample result
        final int outputArity = 2;

        JobSpecification spec = new JobSpecification();

        FileSplit[] storeSalesSplits = new FileSplit[] {new FileSplit(NC1_ID, new FileReference(new File("data/tpch0.001/store_sales1905.tbl"))),};
        IFileSplitProvider storeSalesSplitsProvider = new ConstantFileSplitProvider(storeSalesSplits);
        FileSplit[] webSalesSplits = new FileSplit[] { new FileSplit(NC1_ID, new FileReference(new File("data/tpch0.001/web_sales307.tbl"))) };

        IFileSplitProvider webSalesSplitsProvider = new ConstantFileSplitProvider(webSalesSplits);



        FileScanOperatorDescriptor storeSaleScanner = new FileScanOperatorDescriptor(spec, storeSalesSplitsProvider, new DelimitedDataTupleParserFactory(storeSaleValueParserFactories, '|'), storeSaleDesc);
        PartitionConstraintHelper.addAbsoluteLocationConstraint(spec, storeSaleScanner, NC1_ID);
    ...
    public void testBFsplitTwoNodes() throws Exception {
        //This is to simplify the whole process with just two nodes participating.
        final int outputArity = 2;
        ...
        FileSplit[] webSalesSplits = new FileSplit[] { new FileSplit(NC1_ID, new FileReference(new File(
                "data/tpch0.001/web_sales307.tbl"))) };
 ...
 /*       File file = File.createTempFile(getClass().getName(), "case1");
  ...
  /*  @Test
    ...
    public void customerOrderCIDHybridHashJoin_CaseExtendMultipleJoin() throws Exception {
        ...
        OptimizedHybridHashMultipleJoinOperatorDescriptor join= new OptimizedHybridHashMultipleJoinOperatorDescriptor(spec, 15, 243, 1.2,
                new int[] {4},//table websales
